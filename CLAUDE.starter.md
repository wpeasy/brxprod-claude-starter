...
- **Maps:** the Bricks **Google Map** element renders a single-address map using **Bricks' own bundled key** — you do NOT need to add a site Google Maps API key for a basic/single-location map. (A Google Maps JavaScript API key is only needed for advanced JS-API features / multiple dynamic markers.) The **Leaflet** element (`map-leaflet`) is fully keyless and positions by `center: "lat,lng"` (no geocode-by-address).
