...
- **Bricks gotcha — Block elements also carry a default `row-gap`** (theme default `block._rowGap = var(--brxw-content-gap)`). If you do NOT want a gap between a Block's children (e.g. a flush card where a title sits directly on its content panel), set the Block's **row gap to `0`** (native `_rowGap: "0"` — emits a reliable `#id` rule) or use a **Div** element instead. (A class that sets `gap`/`row-gap` overrides it too.)
