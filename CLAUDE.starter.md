...
- **Background images/media → always use the `brxp-has-bg-media` pattern, never a CSS `background-image`.** Put `.brxp-has-bg-media` on the container (it sets `position:relative; isolation:isolate; overflow:clip`), and add a **direct-child** media element with `.brxp-has-bg-media__media` holding a real Bricks **Image** (or video) — the framework absolutely-positions it (`inset:0; object-fit:cover; z-index:-1000`) so it fills *behind* the container's content. This gives a real `<img>` (lazy-load, `srcset`, `alt`, art-direction, `object-fit`) instead of a CSS background. The container's other children render above it; pair with an overlay and the matching `ally` text colour for contrast over the image.
