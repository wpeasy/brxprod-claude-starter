...
- **CSS classes:** BEM blocks prefixed `nm-` (e.g. `nm-card`, `nm-card__title`, `nm-card--featured`). Atomic **sub-components** designed to be embedded inside other components (eyebrow, button group, label chip, etc.) use the `atom-` prefix instead (e.g. `atom-eyebrow`, `atom-eyebrow--primary`) — they are library-level, not project-bound, and never use `nm-`.
...
- **Heading FIRST in source order; eyebrows / kickers / labels / taglines come AFTER it.** Any small label shown *visually above* a heading (an eyebrow like "AI VIDEO PLATFORM", a category kicker, a tagline) MUST come **after** the heading in the DOM and be lifted into view with CSS `order` (e.g. `order: -1` on the eyebrow, or reorder via the flex/grid parent). Screen readers and keyboard users follow source order, and a section's heading should be the *first* thing encountered inside it — it's also the element `aria-labelledby` names. This is the same content-first principle as the "image last" rule, applied to text. **Author order:** `heading` → `eyebrow` → `subtitle`, then place the eyebrow on top visually. Never emit `eyebrow` → `heading` in source. (Applies to heroes, section heads, feature rows, cards — anywhere a kicker sits above a title.)
...
### Bricks styling
- **Don't add or change styles that weren't requested.** Modern sites are built on CSS frameworks — rely on the `brxw-*`/`brxp-*` tokens + the Bricks **Theme Style** defaults and only override a value with a **specific, justified reason** (e.g. an a11y contrast pairing the theme doesn't supply). Default to leaving the element's `nm-` global class **empty** and let inheritance / the framework do the work; **verify on the frontend** rather than pre-emptively styling. Never add decorative defaults (underlines, hovers, offsets, colours) on a whim.
...
- **Background images/media → always use the `brxp-has-bg-media` pattern, never a CSS `background-image`.** Put `.brxp-has-bg-media` on the container (it sets `position:relative; isolation:isolate; overflow:clip`), then add a real Bricks **Image** (or video) with `.brxp-has-bg-media__media` **directly on it** (no wrapping div) as the container's **last child** — decorative media goes last in source order. The framework absolutely-positions it (`inset:0; object-fit:cover; z-index:-1000`) behind the content, giving a real `<img>` (lazy-load, `srcset`, `object-fit`) instead of a CSS background. **The pairing is mandatory:** `.brxp-has-bg-media` on a container is only half the pattern — its media child (image, video, **or a shader `<canvas>`**) MUST also carry `.brxp-has-bg-media__media`, which is the class that actually positions it (`position:absolute; inset:0; object-fit:cover; z-index:-1000`). A container with `.brxp-has-bg-media` but no `__media` on its media child is a bug (the media is never placed) — and when `__media` is present you don't need a custom absolute-position class on the media. **Mark it decorative:** empty `alt` + `aria-hidden="true"`. Pair with an overlay + the matching `a11y` text colour for contrast. (BRXProd's CSS also supports a `.brxp-has-bg-media__media` *wrapper* containing an `<img>` via its `> img` rule, but prefer the class directly on the image.) **Above-the-fold (hero) backgrounds → set the Image's `loading: eager`:** it's the LCP image, and Bricks' default lazy-load otherwise leaves a blank placeholder behind the content until JS swaps the real `src` in (a behind-the-content image often never visibly triggers).
- **Accessible text colors (A11Y pairing):** every *solid* `brxp` color variant has a matching accessible text-color variable for use as the foreground when that color is the **background**. Build the name by inserting `a11y-` after `brxp-` and appending `-text`:
  - ⚠️ **Spelling: it is `a11y` (a-one-one-y), NOT `ally` (a-L-L-y).** The two are visually identical in many fonts — a known trap. The token is `--brxp-a11y-<role>-text` (emitted inline by BRXProd). Each solid `brxp` colour has exactly one a11y text pair and **the pair differs per role** (e.g. info/success/warning text = `#1a1a1a` dark, danger text = `#f5f5f5` light) — always pull from the token, never guess.
...
- **Use BEM class naming** for new classes. **All component root classes MUST use the `/* Settings */` public → private CSS variable bridge pattern** (documented in [`BRICKS-COMPONENTS.md`](BRICKS-COMPONENTS.md)): a `/* Settings */` block maps public override vars (`--nm-block-*`) to private consumption vars (`--_nm-block-*` with fallbacks to design tokens), then all CSS rules consume only the private vars. Child element classes consume private vars only; variant modifier classes set only public vars. **No literal values in CSS** — every value must flow through a token (`var(--brxw-*)` / `var(--brxp-*)`) or a private var.
...
- **Width comes from Rails (the band), not `max-width`. (MUST — enforced by the linter.)** Every section's *outer* horizontal structure comes from a Rails band, **not** from a `container`'s theme max-width and **never** from `max-width` / `margin-inline: auto` / `margin: auto` on a layout wrapper. **Banned in `_cssCustom` on any layout class:** `max-width`, `margin-inline: auto`, `margin: auto` used to size/centre a block. The **only** allowed `max-width` is a *typographic measure on a text element itself* (a heading/paragraph), via `brxw-text-width-*` / `brxw-title-width-*` tokens — never on a wrapper/`__head`/`__inner`. To centre an intro block, centre it *with the band + `text-align`/`align-items`*, and cap the **text elements** with a text-width token — do not wrap it in a max-width+auto-margin div.
  - **Data-layer Rails recipe (apply on every section):** put **`brxp-rails`** in the section's `_cssGlobalClasses`; then place the section's **direct child** (the inner wrapper) into a band on its own `nm-` class via named grid lines — e.g. `%root% { grid-column-start: content-start; grid-column-end: content-end; }` (or `wide-start/-end`, `breakout-*`, `layout-*`, `full-*`). A full-bleed coloured band = section background spans full width (sections are full width) while the inner wrapper sits in the `content` band. Wider media/grids → promote that child to `wide`/`breakout`. Internal card/column layout still uses flex/`auto-fit` grids — Rails is for the band only.
- **Use native CSS nesting** for pseudo-elements, states, and direct child selectors (`&::before`, `&::after`, `&:hover`, `&:focus-visible`, `& > .child`). **Never write flat descendant-selector chains** (`.nm-block .nm-block__title { … }`) when nesting achieves the same result. Keep nesting shallow — max 2 levels deep.
...
- **Outer width via Rails, not `max-width`** — every `section` carries `brxp-rails` and places its inner wrapper in a band; no layout class sets `max-width` / `margin-inline: auto` / `margin: auto` to size or centre a block (text-measure `max-width` on a text element is the only exception).
- **Eyebrow/kicker after the heading** — flags a non-heading text element that appears *before* a sibling heading under the same parent (it should come after, lifted with CSS `order`).
- **Reused tree → Component** — flags structurally-repeated element subtrees (same shape ≥2×) as Bricks-Component candidates; a tree reused across pages (CTA band, section header, card) MUST be a component, not copy-pasted classes.
...
- **Match the pattern to its native Bricks element BEFORE hand-rolling.** Bricks ships real elements for most recognizable UI — use them instead of faking the effect with text/divs:
  ...
  - Logo wall / partner strip → **`carousel`** or **`slider`** (not a static `<ul>` of names).
  - FAQ / disclosure set → **`accordion-nested`**; tabbed content → **`tabs-nested`**; quote/media slider → **`slider-nested`**.
  - Icon + heading + text tile → `icon-box` (or a hand-built tile only when you need custom structure).
  - **Hand-roll only when** the content is a **CMS query loop** (testimonials/team/projects bound to a CPT — the native `testimonials`/`team-members` elements don't bind to your CPT fields, so a loop is correct there) **or** no native element expresses it. When a design/reference shows a recognizable component (carousel, counter, accordion, tabs), reproduce it with the matching element — don't silently downgrade it to static markup.
  - Discover the exact element + controls with `bricks-list-elements element="<name>"` before using; verify on the frontend (e.g. a `counter` actually counts).
- **Placeholder assets → generate inline SVG, never substitute text.** When a design needs logos/illustrations you don't have, build lightweight **`svg`** elements (dummy wordmark logos, abstract marks) rather than typing the brand name as text. Keep them decorative (`aria-hidden="true"`, sign code with `wp_hash` per the SVG recipe). A "logo" rendered as a `<span>` of text is a bug, not a placeholder.
- **Reusable UI → atomic structures, not page-scoped BEM.** Anything broadly used (notice / info box, badge, pill, button group, media object, card) gets its **own block class** (e.g. `nm-info-box`), **never** a page-bound `nm-<page>__thing`. Build it once, apply it anywhere.
  - **Variants → BEM modifier classes driven by the block's `/* Settings */` public vars.** Default on the base class; each variant is a **separate global class** that **only re-sets the public vars** — a `/* Settings */` comment block with var declarations and nothing else (no direct CSS properties, no re-declarations). E.g. `nm-info-box` (default `info`) + `nm-info-box--success` / `--warning` / `--danger` (each sets `--nm-info-box-bg` + the matching **`--brxp-a11y-<role>-text`**) + density modifiers like `--compact`. **Always pull the a11y pair from the token, never guess** — it differs per role (success/warning text dark, danger text light). Apply the base **and** the modifier together.
  - **Promote to a Bricks Component when the reusable unit is an element TREE** (icon + heading + text + CTA, testimonial card, CTA band) instantiated across pages and edited centrally. Rule of thumb: **single styled element → global class + BEM modifiers (atomic); reusable multi-element structure → Bricks Component.** Never copy-paste element trees. **This is not optional polish:** if the same tree appears on ≥2 pages (a CTA band, a section header), copy-pasting it is a Definition-of-Done failure — make the component. Don't let the component-creation caveats below scare you out of it.
  - **Build components with the Novamira component abilities**, not raw `bricks_components` writes: `bricks-create-component` (generates the id + remaps refs — **the root element's id becomes the component id**), `bricks-apply-component` (places an instance pointer via `cid`), `bricks-list-components` / `bricks-edit-component` / `bricks-delete-component`. Variants ride a **`class`-type property** whose `options[].value` carry modifier-class ids. A **`slot` element** holds instance content — the instance element carries `cid` + `properties` + `slotChildren:{slotId:[childIds]}`, and the slot children are ordinary page elements parented to the instance. *Caveats:* a property can't connect to the **root** at create time (root id is rewritten mid-create — create body first, then attach the root-connected property by the real id; this needs a small **data-layer write**, because `bricks-edit-component` patches label/category/elements only, **not** properties); keep element ids **≤ 6 chars** (Bricks truncates longer ids in render, breaking `slotChildren` key matching).
  - **Minimum-viable component (the cheap, low-risk path):** (1) build the body tree once and `bricks-create-component` from it (or from an existing page subtree via `from_post_id`+`from_element_id`); (2) for each per-instance text/value, add a **`text`-type property** and connect it to that element's setting (e.g. heading `text`, button `text`/`link`) — instances then just pass `properties: {propId: "…"}`, no re-authoring; (3) for **CMS** cards, skip properties and let the body use `{dynamic-data}` tokens, placing one instance inside the query-loop item; (4) `bricks-apply-component` to drop instances. Reach for `slot`s only when an instance needs to inject arbitrary child elements (rare) — for plain text/links, properties are simpler. Remember the caveats: root-connected properties are attached *after* create via a small data-layer write; keep element ids ≤ 6 chars.
  - **Property types quick-ref:** `text`/`textarea` → `settings.text`; `select` → native Bricks enums (`tag` for heading level, `style`/`size` for buttons); `toggle` → `_hideElementBuilder` (builder canvas only) and/or `_hideElementFrontend` (live site); `class` → `_cssGlobalClasses` (applies variant modifier class IDs, use `multiple:true` for stackable variants); `link` → `settings.link`; `icon` → `settings.icon`. **Always set `group`** on every property — properties sharing a group appear as a labelled section in the panel; ungrouped properties pile up in a flat list. **Full property reference + worked examples → [`BRICKS-COMPONENTS.md`](BRICKS-COMPONENTS.md).**
  - **Sub-components inside component bodies** use the `atom-` namespace — they are embedded as `cid` pointer elements within the parent component's element tree. To drive a sub-component's variant from the parent: either (a) connect a `class`-type property on the parent to the sub-component instance element's `_cssGlobalClasses`, or (b) set the sub-component's public CSS vars (e.g. `--atom-eyebrow-color`) on the parent component's root class or a parent variant class — the CSS cascade carries them down automatically through the `/* Settings */` bridge.
  - **Component JS → a Bricks Code element INSIDE the component (a code block), NEVER a Fluent Snippet** — we can't assume Fluent Snippets (or any snippet plugin) is installed, and the JS must travel with the component. Use the Code element's **`javascriptCode`** field with **`executeCode: true`** (the code-signature gate is only for the PHP `code` field, not `javascriptCode`/`cssCode`; without `executeCode` the code renders as a visible listing). Write it as an **IIFE with multi-instance protection** — a one-time init guard (`if (window.__nmX) return; window.__nmX = true;`) + event delegation — so N instances bind once, not N times.
  - **Keep the Code element OUTSIDE any flex/grid wrapper.** The builder renders a Code element as a real, space-taking box even with `executeCode` on (it's only an invisible `<script>` on the frontend), so it squeezes a flex row in the editor. Wrap the visual children in an inner row and keep the code element a plain **sibling** of that row.
  - **Drive a `data-*` attribute from a property** (toggles that CSS *and* JS read): define the attribute on the connected element's `_attributes` (`[{id, name, value}]`) and point the property's connection at the key **`_attributes|<attr_id>|value`** — the value flows into that attribute (e.g. `Dismissible` → `data-dismissible="true|false"`; CSS `[data-dismissible="false"] .x{display:none}` + the JS reads it). **Use property type `select`, NOT `toggle`** — `toggle` *clears* the connected setting when off, so it can't hold a value.
  - **UI icons (close / chevron / status) → inline `<svg>` via the Bricks `svg` element (`source:"code"`), not a text glyph or icon-font char.** The code source is signature-gated — sign server-side with **`signature: wp_hash(<svg markup>)`** (verified as `wp_hash(code)===signature`, code-only, so valid across all instances). Size in `em`, and set colour via the **`svg` element's own `stroke`/`fill` controls** (`{raw:"currentColor"}`) — they emit CSS onto ` :not([stroke="none"])` / ` :not([fill="none"])`, so they stay builder-editable and need no re-signing (`currentColor` inherits the host's a11y text colour). Wrap a clickable icon in a real `<button>` (`customTag:button` + `aria-label`).
...
- ⚠️ **Never delete `uploads/bricks/css/style-manager.min.css` to "force a refresh."** User `nm-` global-class CSS is emitted **inline per page** (in the page's `<style id="bricks-frontend-inline-inline-css">`), **not** in that file. The file holds palette colour vars + scale vars + utility classes, generated by `Bricks\Ajax::generate_style_manager_css_file()` (reads the colour palette + global-variable categories); the `--brxp-a11y-*` vars are injected inline by BRXProd. Deleting it does **not** refresh class CSS and drops nothing recoverable from that file alone. (A wrong-rendered class is almost always a **duplicate global-class id**, not a cache — see the unique-id rule above.)
- **Code-execution gate:** the PHP `code` field of a Code/SVG element requires a `signature` = `wp_hash(code)` (`Helpers::verify_code_signature` checks `wp_hash(code) === signature`, hashing the **code only**, so one signature is valid across every instance/post); `javascriptCode`/`cssCode` do **not** need a signature, but need **`executeCode: true`** to OUTPUT (as `<script>`/`<style>`) instead of rendering as a visible code listing.
