...
- **A card tucked into a panel's corner joins via OUTSETS on the card's corners that face into the panel** — e.g. a card in the panel's **bottom-left** flares **up** where its top meets the panel and **right** where its side meets it (`top-right` ::after + `bottom-right` ::before). Don't reduce it to a single rounded card corner (that reads as a floating card, not a notch); read the design for *which* corners cut into the panel, and confirm on the render.
