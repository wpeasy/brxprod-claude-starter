...
5. **Write back:**
   - class → `novamira/bricks-edit-global-class` with the merged `settings._cssCustom` (author the **literal `.class-name`** selector — NOT `%root%`, native `&::before` nesting, pretty-printed). If the class is missing, create it with `novamira/bricks-create-global-class`.
...
- ⚠️ **Author the literal `.class-name`, never `%root%`.** Bricks' **frontend CSS generator does NOT resolve `%root%`** for data-layer writes — a `%root%`-rooted `_cssCustom` emits verbatim and **won't apply on the frontend** until the class is re-saved in the builder. So write `.nm-faq { … }` / `.nm-faq::before { … }`. The builder still shows it as `%root%` and round-trips fine. On **read-back, expect the literal** — detect the existing rule / `::before` against `.class-name` and merge into the stored literal root.
